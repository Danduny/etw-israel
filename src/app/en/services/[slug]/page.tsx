...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const service = serviceDetails.en[slug as keyof typeof serviceDetails.en];
  if (!service) return {};
  return {
...
export default async function ServicePageEN({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const serviceItem = servicesList.en.find((s) => s.slug === slug);
    ...
    <main>
      ...
      <section className="pt-32 pb-20 md:pt-40 md:pb-28 relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto px-6 relative z-10">
          <Link href="/en/#services" className="text-brand text-sm font-medium hover:text-brand-light transition-colors mb-6 inline-block">
      ...
      <section className="py-20 md:py-28 relative overflow-hidden bg-deep-purple">
        ...
        <div className="max-w-4xl mx-auto px-6 relative z-10">
          ...
          <h2 className="text-3xl md:text-4xl leading-[1.1] tracking-tight mb-12 font-extrabold">
            Everything in <span className="text-gradient">One Package</span>
          </h2>
          ...
          <div className="space-y-6">
            {service.process.map((p, i) => (
              <div key={i} className="glass-card-gradient rounded-xl p-8 flex gap-6 items-start">
                <span className="text-brand font-extrabold text-2xl shrink-0" style={{ fontFamily: "Inter" }}>
                  {p.step}
                </span>
                <div>
      ...
      <section className="py-20 md:py-28 relative overflow-hidden bg-aurora">
        ...
        <div className="max-w-4xl mx-auto px-6 relative z-10 text-center">
          ...
          <p className="text-5xl md:text-6xl font-extrabold text-gradient mb-6">
            {service.price}
          </p>
          <p className="text-gray-400 text-lg mb-10 max-w-xl mx-auto font-light">
            No surprises. No hidden costs. A short call and we&apos;ll know exactly what you need.
          </p>
          <a
